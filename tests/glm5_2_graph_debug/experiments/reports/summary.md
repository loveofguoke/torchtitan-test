...
| TP `aten.complex` strategy | opt-in DTensor pointwise strategy 注册 | TorchTitanTurbo `tools/graph_compat.py` |
| PP metadata 通信 | metadata object P2P 改为 `use_batch=False` | TorchTitanTurbo `tools/graph_compat.py` |
| EP 空 expert/grouped-mm | 空组补行/全空 bypass/显式 backward/int32 offsets | TorchTitanTurbo `tools/graph_compat.py` |
| 零长度 Triton kernel | base 与 symbolic grouped autotuner 的 `*numel=0` guard | TorchTitanTurbo `tools/graph_compat.py` |
| NPUGraph replay 默认关闭 | `TORCHTITAN_NPUGRAPH_SKIP_ALL=1` | common + TorchTitanTurbo graph compat |
