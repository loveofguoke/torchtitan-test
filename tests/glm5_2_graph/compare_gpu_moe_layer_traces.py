#!/usr/bin/env python3
"""Compare eager and cold-Inductor ordered MoE-layer forward traces."""
...
from tests.glm5_2_graph.gpu_moe_layer_trace import MOE_STAGE_NAMES  # noqa: E402
...
def compare_moe_layer_traces(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    return {
        "schema": "torchtitan.glm5_2.gpu_moe_layer_trace_comparison",
        "paths": {name: str(path) for name, path in paths.items()},
        "stage_order": list(MOE_STAGE_NAMES),
        "comparisons": [
...
def main() -> None:
    ...
    for comparison in result["comparisons"]:
        ...
        if first is None:
            print("  all Layer 1 MoE stages exact")
        else:
            print(
                f"  first={first['stage']} "
