#!/usr/bin/env python3
"""Compare ordered CUDA eager and cold-Inductor attention-stage traces."""
...
from tests.glm5_2_graph.gpu_attention_internal_trace import (  # noqa: E402
...
def _compare_pair(
    ...
    for index, stage in enumerate(ATTENTION_STAGE_NAMES):
        ...
        rows.append(row)
    first_divergence = next((row for row in rows if not row["exact"]), None)
    return {
        "left": left_name,
        "right": right_name,
        "records": rows,
...
def compare_attention_traces(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    return {
        "schema": "torchtitan.glm5_2.gpu_attention_trace_comparison",
        "paths": {name: str(path) for name, path in paths.items()},
        "stage_order": list(ATTENTION_STAGE_NAMES),
        "comparisons": [
...
def main() -> None:
    ...
    for comparison in result["comparisons"]:
        ...
        if first is None:
            print("  all attention stages exact")
            continue
        print(
            f"  first={first['stage']} "
