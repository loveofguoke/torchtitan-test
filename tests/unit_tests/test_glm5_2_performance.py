...
class TestPerformanceConfig(unittest.TestCase):
    ...
    def test_presets_progress_from_overview_to_runtime(self):
        ...
        self.assertFalse(presets["flamegraph"].with_stack)
        self.assertFalse(presets["flamegraph"].with_modules)
        self.assertEqual(presets["flamegraph"].parse_mode, "offline")
        self.assertFalse(presets["flamegraph"].export_stacks)
        self.assertFalse(presets["runtime"].profile_memory)
        self.assertFalse(presets["runtime"].with_stack)
        self.assertTrue(presets["runtime"].op_attr)
        self.assertEqual(presets["runtime"].gc_detect_threshold, 1.0)
        self.assertTrue(presets["operator"].with_flops)
        self.assertTrue(presets["operator"].op_attr)
        self.assertFalse(presets["operator"].record_op_args)
        self.assertTrue(presets["memory"].profile_memory)
        self.assertFalse(presets["memory"].with_stack)
        self.assertFalse(presets["memory"].export_memory_timeline)
        self.assertEqual(presets["system"].host_system, ("cpu", "mem"))
        self.assertFalse(presets["system"].system_io)
        self.assertFalse(presets["system"].mstx)
    ...
    def test_preset_exports_official_profiler_controls(self):
        ...
        self.assertEqual(environment[prefix + "WITH_MODULES"], "false")
        self.assertNotIn(prefix + "EXPORT_STACKS", environment)
        self.assertNotIn(prefix + "EXPORT_MEMORY_TIMELINE", environment)
