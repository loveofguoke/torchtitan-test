#!/usr/bin/env python3
"""GPU eager msProbe single-card versus distributed diagnostic entry."""
...
def _param_dtype() -> str:
    value = os.environ.get("GLM5_GPU_EAGER_PARAM_DTYPE", "bfloat16")
    if value not in {"float32", "bfloat16"}:
        raise ValueError(
            "GLM5_GPU_EAGER_PARAM_DTYPE must be float32 or bfloat16"
        )
    return value


STEPS = _positive_integer("GLM5_GPU_EAGER_ALIGNMENT_STEPS", "1")
GLOBAL_BATCH_SIZE = _positive_integer("GLM5_GPU_EAGER_GLOBAL_BATCH_SIZE", "16")
PARAM_DTYPE = _param_dtype()
ALL_DEVICES = os.environ.get("GLM5_GPU_EAGER_CUDA_DEVICES", "0,1,2,3")
REFERENCE_DEVICE = os.environ.get("GLM5_GPU_EAGER_REFERENCE_CUDA", "0")

CONFIG = FormalExperimentConfig(
    name="glm5-2-gpu-eager-topology-msprobe",
    ...
    fixture_root="gpu_eager_msprobe_fixtures",
    artifact_root="gpu_eager_msprobe_artifacts",
    report_root="gpu_eager_msprobe_reports",
    run_root="gpu_eager_msprobe_runs",
    shared_reference_group=(
        os.environ.get("GLM5_GPU_EAGER_SHARED_REFERENCE_GROUP")
